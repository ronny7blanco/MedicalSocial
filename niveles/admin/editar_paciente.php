...
//Seleccionamos los campos de la tabla que necesitamos
$dui2 = $_GET['dui'];
$sentencia2=  $conn->query("SELECT * FROM pacientes where dui='$dui2'");
...
<h2>EDITAR DOCTOR</h2>
        ...
        <form  enctype="multipart/form-data"  name="form_login" method="post">
            ...
            <td><input type="text" required name="dui" id="dui" value="<?php echo $row2['dui']; ?>" /></td>
            </tr>
            ...
            <td><input type="text" required name="nickname" id="nickname" value="<?php echo $row2['nickname']; ?>" /></td>
            </tr>

            <tr>
            <td  id="alinear">Género:</td>
            <td><input type="text" required name="genero" id="genero" value="<?php echo $row2['genero']; ?>" /></td>
            </tr>

            <tr>
            <td  id="alinear">Estado Civil:</td>
            <td><input type="text" required name="estado_civil" id="estado_civil" value="<?php echo $row2['estado_civil']; ?>" /></td>
            </tr>
 ...
 if(isset($_POST["boton"])==true)
  {
    ...
    //Sentencia SQL que se ejecuta, se agregan los campos del formulario a los campos de la tabla de la base de datos
     $stmt = $conn->prepare('UPDATE pacientes SET dui= ?, nombre= ?, apellido= ?, nit= ?, direccion= ?, telefono= ?, email= ?, password= ?, genero= ?, estado_civil= ?  WHERE dui= ?');
    ...
    $stmt->bindParam(9, $genero);
    $stmt->bindParam(10, $estado_civil);
    $stmt->bindParam(11, $dui_S);
    ...
    location.href='buscar_paciente.php';
    </script>
    <?php
}
?>
